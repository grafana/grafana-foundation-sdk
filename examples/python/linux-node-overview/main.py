...
if __name__ == '__main__':
    builder = (
        ...
        # "Cluster" variable
        .with_variable(
            common.query_variable("cluster", "Cluster", 'label_values(node_uname_info{job=~"integrations/(node_exporter|unix)"}, cluster)')
            .all_value(".*")
        )
        # "Job" variable
        .with_variable(
            common.query_variable("job", "Job", 'label_values(node_uname_info{job=~"integrations/(node_exporter|unix)",cluster=~"$cluster"}, job)')
            .all_value(".+")
        )
        # "Instance" variable
        .with_variable(
            common.query_variable("instance", "Instance", 'label_values(node_uname_info{job=~"integrations/(node_exporter|unix)",cluster=~"$cluster",job=~"$job"}, instance)')
            .all_value(".+")
        )
        # Panels
