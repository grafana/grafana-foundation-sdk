...
```php
defaults(\Grafana\Foundation\Dashboardv2beta1\FieldConfig $defaults)
```
...
```php
fieldConfig(\Grafana\Foundation\Dashboardv2beta1\FieldConfigSource $fieldConfig)
```
...
```php
spec(\Grafana\Foundation\Dashboardv2beta1\VizConfigSpec $spec)
```
