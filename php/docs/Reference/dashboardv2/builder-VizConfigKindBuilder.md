...
```php
defaults(\Grafana\Foundation\Dashboardv2\FieldConfig $defaults)
```
...
```php
fieldConfig(\Grafana\Foundation\Dashboardv2\FieldConfigSource $fieldConfig)
```
...
```php
nullValueMode(\Grafana\Foundation\Dashboardv2\NullValueMode $nullValueMode)
```
...
```php
spec(\Grafana\Foundation\Dashboardv2\VizConfigSpec $spec)
```
