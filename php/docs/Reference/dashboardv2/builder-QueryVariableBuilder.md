...
```php
refresh(\Grafana\Foundation\Dashboardv2\VariableRefresh $refresh)
```
...
```php
regexApplyTo(\Grafana\Foundation\Dashboardv2\VariableRegexApplyTo $regexApplyTo)
```
...
```php
sort(\Grafana\Foundation\Dashboardv2\VariableSort $sort)
```
...
```php
staticOptionsOrder(\Grafana\Foundation\Dashboardv2\QueryVariableSpecStaticOptionsOrder $staticOptionsOrder)
```
