---
title: <span class="badge builder"></span> AutoGridBuilder
---
# <span class="badge builder"></span> AutoGridBuilder

## Constructor

```php
new AutoGridBuilder()
```
## Methods

### <span class="badge object-method"></span> build

Builds the object.

```php
build()
```

### <span class="badge object-method"></span> columnWidth

```php
columnWidth(float $columnWidth)
```

### <span class="badge object-method"></span> columnWidthMode

```php
columnWidthMode(\Grafana\Foundation\Dashboardv2\AutoGridLayoutSpecColumnWidthMode $columnWidthMode)
```

### <span class="badge object-method"></span> fillScreen

```php
fillScreen(bool $fillScreen)
```

### <span class="badge object-method"></span> item

@param \Grafana\Foundation\Cog\Builder<\Grafana\Foundation\Dashboardv2\AutoGridLayoutItemKind> $item

```php
item(\Grafana\Foundation\Cog\Builder $item)
```

### <span class="badge object-method"></span> items

@param array<\Grafana\Foundation\Cog\Builder<\Grafana\Foundation\Dashboardv2\AutoGridLayoutItemKind>> $items

```php
items(array $items)
```

### <span class="badge object-method"></span> maxColumnCount

```php
maxColumnCount(float $maxColumnCount)
```

### <span class="badge object-method"></span> rowHeight

```php
rowHeight(float $rowHeight)
```

### <span class="badge object-method"></span> rowHeightMode

```php
rowHeightMode(\Grafana\Foundation\Dashboardv2\AutoGridLayoutSpecRowHeightMode $rowHeightMode)
```

### <span class="badge object-method"></span> withItem

```php
withItem(string $name)
```

## See also

 * <span class="badge object-type-class"></span> [AutoGridLayoutKind](./object-AutoGridLayoutKind.md)
