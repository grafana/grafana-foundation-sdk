...
final class HttpRequestMethod implements \JsonSerializable, \Stringable {
    ...
    {
        if (!isset(self::$instances["DELETE"])) {
            self::$instances["DELETE"] = new self("DELETE");
        }
    ...
    {
        if (!isset(self::$instances["PATCH"])) {
            self::$instances["PATCH"] = new self("PATCH");
        }

        return self::$instances["PATCH"];
    }
