...
{
    public static function convert(\Grafana\Foundation\Dashboardv2\CustomVariableKind $input): string
    {
        
        $calls = [
            '(new \Grafana\Foundation\Dashboardv2\CustomVariableBuilder('.\var_export($input->spec->name, true).'))',
        ];
            if ($input->spec->name !== "") {
    ...
    $buffer = 'valuesFormat(';
        $arg0 ='\Grafana\Foundation\Dashboardv2\CustomVariableSpecValuesFormat::fromValue("'.$input->spec->valuesFormat.'")';
        $buffer .= $arg0;
