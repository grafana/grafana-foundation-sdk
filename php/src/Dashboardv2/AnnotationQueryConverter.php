...
{
    public static function convert(\Grafana\Foundation\Dashboardv2\AnnotationQueryKind $input): string
    {
        
        $calls = [
            '(new \Grafana\Foundation\Dashboardv2\AnnotationQueryBuilder())',
        ];
    ...
    $buffer = 'filter(';
        $arg0 = \Grafana\Foundation\Dashboardv2\AnnotationPanelFilterConverter::convert($input->spec->filter);
        $buffer .= $arg0;
    ...
    $buffer = 'mappings(';
        ...
        foreach ($input->spec->mappings as $key => $arg1) {
            $tmpmappingsarg1 = \Grafana\Foundation\Dashboardv2\AnnotationEventFieldMappingConverter::convert($arg1);
