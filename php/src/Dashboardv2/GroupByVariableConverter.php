...
{
    public static function convert(\Grafana\Foundation\Dashboardv2\GroupByVariableKind $input): string
    {
        
        $calls = [
            '(new \Grafana\Foundation\Dashboardv2\GroupByVariableBuilder('.\var_export($input->spec->name, true).'))',
        ];
            if ($input->group !== "") {
    ...
    $buffer = 'datasource(';
        $arg0 = \Grafana\Foundation\Dashboardv2\Dashboardv2GroupByVariableKindDatasourceConverter::convert($input->datasource);
        $buffer .= $arg0;
    ...
    $buffer = 'defaultValue(';
        $arg0 ='(new \Grafana\Foundation\Dashboardv2\VariableOption('.(($input->spec->defaultValue->selected !== null) ? 'selected: '.\var_export($input->spec->defaultValue->selected, true).', ' : '').'text: '.\var_export($input->spec->defaultValue->text, true).',value: '.\var_export($input->spec->defaultValue->value, true).','.(($input->spec->defaultValue->properties !== null) ? 'properties: '.\var_export($input->spec->defaultValue->properties, true).', ' : '').'))';
        $buffer .= $arg0;
