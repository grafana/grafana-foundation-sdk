...
final class PanelConverter
{
    public static function convert(\Grafana\Foundation\Dashboard\Panel $input): string
    {
        ...
        $calls = [
            '(new \Grafana\Foundation\News\PanelBuilder())',
        ];
            if ($input->id !== null) {
    ...
    $calls[] = $buffer;
            ...
            if ($input->options !== null && $input->options instanceof \Grafana\Foundation\News\Options && $input->options->feedUrl !== null && $input->options->feedUrl !== "") {
    
        
    $buffer = 'feedUrl(';
        $arg0 =\var_export($input->options->feedUrl, true);
        $buffer .= $arg0;
    ...
    $calls[] = $buffer;
            ...
            if ($input->options !== null && $input->options instanceof \Grafana\Foundation\News\Options && $input->options->showImage !== null && $input->options->showImage !== true) {
    
        
    $buffer = 'showImage(';
        $arg0 =\var_export($input->options->showImage, true);
        $buffer .= $arg0;
