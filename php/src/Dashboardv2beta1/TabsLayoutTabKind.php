<?php

namespace Grafana\Foundation\Dashboardv2beta1;

class TabsLayoutTabKind implements \JsonSerializable
{
    public string $kind;

    public \Grafana\Foundation\Dashboardv2beta1\TabsLayoutTabSpec $spec;

    /**
     * @param \Grafana\Foundation\Dashboardv2beta1\TabsLayoutTabSpec|null $spec
     */
    public function __construct(?\Grafana\Foundation\Dashboardv2beta1\TabsLayoutTabSpec $spec = null)
    {
        $this->kind = "TabsLayoutTab";
    
        $this->spec = $spec ?: new \Grafana\Foundation\Dashboardv2beta1\TabsLayoutTabSpec();
    }

    /**
     * @param array<string, mixed> $inputData
     */
    public static function fromArray(array $inputData): self
    {
        /** @var array{kind?: string, spec?: mixed} $inputData */
        $data = $inputData;
        return new self(
            spec: isset($data["spec"]) ? (function($input) {
    	/** @var array{title?: string, layout?: mixed|mixed|mixed|mixed, conditionalRendering?: mixed, repeat?: mixed} */
    $val = $input;
    	return \Grafana\Foundation\Dashboardv2beta1\TabsLayoutTabSpec::fromArray($val);
    })($data["spec"]) : null,
        );
    }

    /**
     * @return mixed
     */
    public function jsonSerialize(): mixed
    {
        $data = new \stdClass;
        $data->kind = $this->kind;
        $data->spec = $this->spec;
        return $data;
    }
}
