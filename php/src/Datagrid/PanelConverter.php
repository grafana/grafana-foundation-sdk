...
final class PanelConverter
{
    public static function convert(\Grafana\Foundation\Dashboard\Panel $input): string
    {
        ...
        $calls = [
            '(new \Grafana\Foundation\Datagrid\PanelBuilder())',
        ];
            if ($input->id !== null) {
    ...
    $calls[] = $buffer;
            ...
            if ($input->options !== null && $input->options instanceof \Grafana\Foundation\Datagrid\Options && $input->options->selectedSeries !== 0) {
    
        
    $buffer = 'selectedSeries(';
        $arg0 =\var_export($input->options->selectedSeries, true);
        $buffer .= $arg0;
