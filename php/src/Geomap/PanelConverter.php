...
final class PanelConverter
{
    public static function convert(\Grafana\Foundation\Dashboard\Panel $input): string
    {
        ...
        $calls = [
            '(new \Grafana\Foundation\Geomap\PanelBuilder())',
        ];
            if ($input->id !== null) {
    ...
    $calls[] = $buffer;
            ...
            if ($input->options !== null && $input->options instanceof \Grafana\Foundation\Geomap\Options) {
    ...
    $buffer = 'basemap(';
        $arg0 = \Grafana\Foundation\Common\MapLayerOptionsConverter::convert($input->options->basemap);
        $buffer .= $arg0;
    ...
    $calls[] = $buffer;
            ...
            if ($input->options !== null && $input->options instanceof \Grafana\Foundation\Geomap\Options && count($input->options->layers) >= 1) {
    
        
    $buffer = 'layers(';
        $tmparg0 = [];
        foreach ($input->options->layers as $arg1) {
