...
class CanvasElementOptions implements \JsonSerializable
{
    ...
    $val = $input;
    	...
    	/** @var array{top?: float, left?: float, right?: float, bottom?: float, width?: float, height?: float} */
    $val = $input;
    ...
    $val = $input;
    	...
    	/** @var array{color?: mixed, width?: float} */
    $val = $input;
    	return \Grafana\Foundation\Canvas\LineConfig::fromArray($val);
    })($data["border"]) : null,
            connections: array_filter(array_map((function($input) {
    	/** @var array{source?: mixed, target?: mixed, targetName?: string, path?: "straight", color?: mixed, size?: mixed} */
    $val = $input;
