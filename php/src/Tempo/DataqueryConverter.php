...
final class DataqueryConverter
{
    ...
    {
        assert($input instanceof \Grafana\Foundation\Tempo\Dataquery);
        $calls = [
    ...
    $calls[] = $buffer;
            ...
            if ($input->serviceMapQuery !== null) {
    
        
    $buffer = 'serviceMapQuery(';
        switch (true) {
            case is_string($input->serviceMapQuery):
                $disjunctionserviceMapQuery =\var_export($input->serviceMapQuery, true);
                $arg0 = $disjunctionserviceMapQuery;
                break;
            case is_array($input->serviceMapQuery):
                $tmpdisjunctionserviceMapQuery = [];
        foreach ($input->serviceMapQuery as $arg1) {
