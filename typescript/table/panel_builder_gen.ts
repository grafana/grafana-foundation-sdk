...
export class PanelBuilder implements cog.OptionsBuilder<dashboard.Panel> {
    ...
    sortBy(sortBy: cog.OptionsBuilder<common.TableSortByFieldState>[]): this {
		if (!this.internal.options) {
			this.internal.options = table.defaultOptions();
		}
        const sortByResources = sortBy.map(builder => builder.build());
        this.internal.options.sortBy = sortByResources;
        return this;
    }

    // Controls footer options
    footer(footer: cog.OptionsBuilder<common.TableFooterOptions>): this {
		if (!this.internal.options) {
			this.internal.options = table.defaultOptions();
		}
