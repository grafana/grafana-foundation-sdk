---
title: <span class="badge builder"></span> ActionBuilder
---
# <span class="badge builder"></span> ActionBuilder

## Constructor

```typescript
new ActionBuilder()
```
## Methods

### <span class="badge object-method"></span> build

Builds the object.

```typescript
build()
```

### <span class="badge object-method"></span> confirmation

```typescript
confirmation(confirmation: string)
```

### <span class="badge object-method"></span> fetch

```typescript
fetch(fetch: cog.Builder<dashboardv2.FetchOptions>)
```

### <span class="badge object-method"></span> infinity

```typescript
infinity(infinity: cog.Builder<dashboardv2.InfinityOptions>)
```

### <span class="badge object-method"></span> oneClick

```typescript
oneClick(oneClick: boolean)
```

### <span class="badge object-method"></span> style

```typescript
style(style: {
	backgroundColor?: string;
})
```

### <span class="badge object-method"></span> title

```typescript
title(title: string)
```

### <span class="badge object-method"></span> type

```typescript
type(type: dashboardv2.ActionType)
```

### <span class="badge object-method"></span> variables

```typescript
variables(variables: cog.Builder<dashboardv2.ActionVariable>[])
```

## See also

 * <span class="badge object-type-interface"></span> [Action](./object-Action.md)
