...
```typescript
actions(actions: cog.Builder<dashboardv2.Action>[])
```
...
```typescript
colorScheme(color: cog.Builder<dashboardv2.FieldColor>)
```
...
```typescript
defaults(defaults: dashboardv2.FieldConfig)
```
...
```typescript
fieldConfig(fieldConfig: dashboardv2.FieldConfigSource)
```
...
```typescript
mappings(mappings: dashboardv2.ValueMapping[])
```
...
```typescript
nullValueMode(nullValueMode: dashboardv2.NullValueMode)
```
...
```typescript
overrideByFieldType(fieldType: string, properties: dashboardv2.DynamicConfigValue[])
```
...
```typescript
overrideByName(name: string, properties: dashboardv2.DynamicConfigValue[])
```

### <span class="badge object-method"></span> overrideByQuery

```typescript
overrideByQuery(queryRefId: string, properties: dashboardv2.DynamicConfigValue[])
```
...
```typescript
overrideByRegexp(regexp: string, properties: dashboardv2.DynamicConfigValue[])
```
...
```typescript
spec(spec: dashboardv2.VizConfigSpec)
```
...
```typescript
thresholds(thresholds: cog.Builder<dashboardv2.ThresholdsConfig>)
```
