// Code generated - EDITING IS FUTILE. DO NOT EDIT.

import * as cog from '../cog';
import * as playlistv1 from '../playlistv1';
import * as resource from '../resource';

export class PlaylistBuilder implements cog.Builder<playlistv1.Playlist> {
    protected readonly internal: playlistv1.Playlist;

    constructor(title: string) {
        this.internal = playlistv1.defaultPlaylist();
        this.internal.title = title;
    }

    /**
     * Builds the object.
     */
    build(): playlistv1.Playlist {
        return this.internal;
    }

    title(title: string): this {
        this.internal.title = title;
        return this;
    }

    interval(interval: string): this {
        this.internal.interval = interval;
        return this;
    }

    items(items: cog.Builder<playlistv1.Item>[]): this {
        const itemsResources = items.map(builder1 => builder1.build());
        this.internal.items = itemsResources;
        return this;
    }

    item(item: cog.Builder<playlistv1.Item>): this {
        if (!this.internal.items) {
            this.internal.items = [];
        }
        const itemResource = item.build();
        this.internal.items.push(itemResource);
        return this;
    }
}

/**
 * Creates a resource manifest from a Playlist.
 */
export function manifest(name: string,playlist: cog.Builder<playlistv1.Playlist>): cog.Builder<resource.Manifest> {
	const builder = new resource.ManifestBuilder();
	builder.apiVersion("playlist.grafana.app/playlistv1");
	builder.kind("Playlist");
	builder.metadata(resource.named(name));
	builder.spec(playlist.build());

	return builder;
}

