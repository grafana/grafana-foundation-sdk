...
export class PanelBuilder implements cog.OptionsBuilder<dashboard.Panel> {
    ...
    basemap(basemap: cog.OptionsBuilder<common.MapLayerOptions>): this {
		if (!this.internal.options) {
			this.internal.options = geomap.defaultOptions();
		}
    ...
    layers(layers: cog.OptionsBuilder<common.MapLayerOptions>[]): this {
		if (!this.internal.options) {
			this.internal.options = geomap.defaultOptions();
		}
        const layersResources = layers.map(builder => builder.build());
