...
public class QueryGroupBuilder implements com.grafana.foundation.cog.Builder<QueryGroupKind> {
    ...
    public QueryGroupBuilder targets(List<com.grafana.foundation.cog.Builder<PanelQueryKind>> queries) {
		if (this.internal.spec == null) {
			this.internal.spec = new com.grafana.foundation.dashboardv2.QueryGroupSpec();
		}
	...
	for (PanelQueryKind query : queriesResources) {
		if (query.spec.refId == "") {
			query.spec.refId = String.format("query-%d", this.nextQueryId);
			this.nextQueryId += 1;
		}
	}
	...
	if (querieResource.spec.refId == "") {
		querieResource.spec.refId = String.format("query-%d", this.nextQueryId);
		this.nextQueryId += 1;
	}
