...
public class PanelDeserializer extends JsonDeserializer<Panel> {
    ...
    public Panel deserialize(JsonParser jp, DeserializationContext cxt) throws IOException, JsonProcessingException {
        ...
        if (root.has("maxPerRow")) {
            panel.maxPerRow = mapper.convertValue(root.get("maxPerRow"), new TypeReference<>() {});
        }
        ...
        if (root.has("cacheTimeout")) {
            panel.cacheTimeout = mapper.convertValue(root.get("cacheTimeout"), new TypeReference<>() {});
        }
        if (root.has("queryCachingTTL")) {
            panel.queryCachingTTL = mapper.convertValue(root.get("queryCachingTTL"), new TypeReference<>() {});
        }
        if (root.has("options")) {
