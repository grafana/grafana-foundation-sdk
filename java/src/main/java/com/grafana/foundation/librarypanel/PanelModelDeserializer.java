...
public class PanelModelDeserializer extends JsonDeserializer<PanelModel> {
    ...
    public PanelModel deserialize(JsonParser jp, DeserializationContext cxt) throws IOException, JsonProcessingException {
        ...
        if (root.has("timeCompare")) {
            panelModel.timeCompare = mapper.convertValue(root.get("timeCompare"), new TypeReference<>() {});
        }
        if (root.has("cacheTimeout")) {
            panelModel.cacheTimeout = mapper.convertValue(root.get("cacheTimeout"), new TypeReference<>() {});
        }
        if (root.has("queryCachingTTL")) {
            panelModel.queryCachingTTL = mapper.convertValue(root.get("queryCachingTTL"), new TypeReference<>() {});
        }
        if (root.has("options")) {
