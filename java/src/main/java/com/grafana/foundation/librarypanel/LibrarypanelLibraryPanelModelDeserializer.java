...
public class LibrarypanelLibraryPanelModelDeserializer extends JsonDeserializer<LibrarypanelLibraryPanelModel> {
    ...
    public LibrarypanelLibraryPanelModel deserialize(JsonParser jp, DeserializationContext cxt) throws IOException, JsonProcessingException {
        ...
        if (root.has("cacheTimeout")) {
            librarypanelLibraryPanelModel.cacheTimeout = mapper.convertValue(root.get("cacheTimeout"), new TypeReference<>() {});
        }
        if (root.has("queryCachingTTL")) {
            librarypanelLibraryPanelModel.queryCachingTTL = mapper.convertValue(root.get("queryCachingTTL"), new TypeReference<>() {});
        }
        if (root.has("options")) {
