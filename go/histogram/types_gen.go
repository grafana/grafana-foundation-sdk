...
func (resource *Options) UnmarshalJSONStrict(raw []byte) error {
	...
	// Field "bucketCount"
	if fields["bucketCount"] != nil {
		if string(fields["bucketCount"]) != "null" {
			if err := json.Unmarshal(fields["bucketCount"], &resource.BucketCount); err != nil {
				errs = append(errs, cog.MakeBuildErrors("bucketCount", err)...)
			}

		}
		delete(fields, "bucketCount")
...
func (resource *FieldConfig) UnmarshalJSONStrict(raw []byte) error {
	...
	if fields["stacking"] != nil {
		if string(fields["stacking"]) != "null" {
			...
			resource.Stacking = &common.StackingConfig{}
...
func VariantConfig() variants.PanelcfgConfig {
	return variants.PanelcfgConfig{
		Identifier: "histogram", OptionsUnmarshaler: func(raw []byte) (any, error) {
			options := &Options{}
