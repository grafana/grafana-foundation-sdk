...
func PanelConverter(input dashboard.Panel) string {
	...
	if input.Options != nil && input.Options.(*Options).ColWidth != nil && *input.Options.(*Options).ColWidth != 0.9 {
		...
		arg0 := fmt.Sprintf("%#v", cog.Unptr(input.Options.(*Options).ColWidth))
		buffer.WriteString(arg0)
	...
	if input.FieldConfig.Defaults.Custom != nil && input.FieldConfig.Defaults.Custom.(*FieldConfig).FillOpacity != nil && *input.FieldConfig.Defaults.Custom.(*FieldConfig).FillOpacity != 70 {
