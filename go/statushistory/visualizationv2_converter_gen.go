...
func VisualizationV2Converter(input dashboardv2.VizConfigKind) string {
	calls := []string{
		`statushistory.NewVisualizationV2Builder()`,
	}
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).ColWidth != nil && *input.Spec.Options.(*Options).ColWidth != 0.9 {

		buffer.WriteString(`ColWidth(`)
		arg0 := fmt.Sprintf("%#v", *input.Spec.Options.(*Options).ColWidth)
		buffer.WriteString(arg0)
