// Code generated - EDITING IS FUTILE. DO NOT EDIT.

package cloudwatch

import (
	cog "github.com/grafana/grafana-foundation-sdk/go/cog"
	common "github.com/grafana/grafana-foundation-sdk/go/common"
)

var _ cog.Builder[MetricsQuery] = (*MetricsQueryBuilder)(nil)

// Shape of a CloudWatch Metrics query
type MetricsQueryBuilder struct {
	internal *MetricsQuery
	errors   cog.BuildErrors
}

func NewMetricsQueryBuilder() *MetricsQueryBuilder {
	resource := NewMetricsQuery()
	builder := &MetricsQueryBuilder{
		internal: resource,
		errors:   make(cog.BuildErrors, 0),
	}

	return builder
}

func (builder *MetricsQueryBuilder) Build() (MetricsQuery, error) {
	if err := builder.internal.Validate(); err != nil {
		return MetricsQuery{}, err
	}

	if len(builder.errors) > 0 {
		return MetricsQuery{}, cog.MakeBuildErrors("cloudwatch.metricsQuery", builder.errors)
	}

	return *builder.internal, nil
}

func (builder *MetricsQueryBuilder) RecordError(path string, err error) *MetricsQueryBuilder {
	builder.errors = append(builder.errors, cog.MakeBuildErrors(path, err)...)
	return builder
}

// Whether a query is a Metrics, Logs, or Annotations query
func (builder *MetricsQueryBuilder) QueryMode(queryMode QueryMode) *MetricsQueryBuilder {
	builder.internal.QueryMode = queryMode

	return builder
}

// Whether to use a metric search or metric insights query
func (builder *MetricsQueryBuilder) MetricQueryType(metricQueryType MetricQueryType) *MetricsQueryBuilder {
	builder.internal.MetricQueryType = &metricQueryType

	return builder
}

// Whether to use the query builder or code editor to create the query
func (builder *MetricsQueryBuilder) MetricEditorMode(metricEditorMode MetricEditorMode) *MetricsQueryBuilder {
	builder.internal.MetricEditorMode = &metricEditorMode

	return builder
}

// ID can be used to reference other queries in math expressions. The ID can include numbers, letters, and underscore, and must start with a lowercase letter.
func (builder *MetricsQueryBuilder) Id(id string) *MetricsQueryBuilder {
	builder.internal.Id = id

	return builder
}

// Deprecated: use label
// @deprecated use label
func (builder *MetricsQueryBuilder) Alias(alias string) *MetricsQueryBuilder {
	builder.internal.Alias = &alias

	return builder
}

// Change the time series legend names using dynamic labels. See https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/graph-dynamic-labels.html for more details.
func (builder *MetricsQueryBuilder) Label(label string) *MetricsQueryBuilder {
	builder.internal.Label = &label

	return builder
}

// Math expression query
func (builder *MetricsQueryBuilder) Expression(expression string) *MetricsQueryBuilder {
	builder.internal.Expression = &expression

	return builder
}

// When the metric query type is set to `Insights`, this field is used to specify the query string.
func (builder *MetricsQueryBuilder) SqlExpression(sqlExpression string) *MetricsQueryBuilder {
	builder.internal.SqlExpression = &sqlExpression

	return builder
}

// A unique identifier for the query within the list of targets.
// In server side expressions, the refId is used as a variable name to identify results.
// By default, the UI will assign A->Z; however setting meaningful names may be useful.
func (builder *MetricsQueryBuilder) RefId(refId string) *MetricsQueryBuilder {
	builder.internal.RefId = &refId

	return builder
}

// If hide is set to true, Grafana will filter out the response(s) associated with this query before returning it to the panel.
func (builder *MetricsQueryBuilder) Hide(hide bool) *MetricsQueryBuilder {
	builder.internal.Hide = &hide

	return builder
}

// Specify the query flavor
// TODO make this required and give it a default
func (builder *MetricsQueryBuilder) QueryType(queryType string) *MetricsQueryBuilder {
	builder.internal.QueryType = &queryType

	return builder
}

// AWS region to query for the metric
func (builder *MetricsQueryBuilder) Region(region string) *MetricsQueryBuilder {
	builder.internal.Region = region

	return builder
}

// A namespace is a container for CloudWatch metrics. Metrics in different namespaces are isolated from each other, so that metrics from different applications are not mistakenly aggregated into the same statistics. For example, Amazon EC2 uses the AWS/EC2 namespace.
func (builder *MetricsQueryBuilder) Namespace(namespace string) *MetricsQueryBuilder {
	builder.internal.Namespace = namespace

	return builder
}

// Name of the metric
func (builder *MetricsQueryBuilder) MetricName(metricName string) *MetricsQueryBuilder {
	builder.internal.MetricName = &metricName

	return builder
}

// The dimensions of the metric
func (builder *MetricsQueryBuilder) Dimensions(dimensions Dimensions) *MetricsQueryBuilder {
	builder.internal.Dimensions = &dimensions

	return builder
}

// Only show metrics that exactly match all defined dimension names.
func (builder *MetricsQueryBuilder) MatchExact(matchExact bool) *MetricsQueryBuilder {
	builder.internal.MatchExact = &matchExact

	return builder
}

// The length of time associated with a specific Amazon CloudWatch statistic. Can be specified by a number of seconds, 'auto', or as a duration string e.g. '15m' being 15 minutes
func (builder *MetricsQueryBuilder) Period(period string) *MetricsQueryBuilder {
	builder.internal.Period = &period

	return builder
}

// The ID of the AWS account to query for the metric, specifying `all` will query all accounts that the monitoring account is permitted to query.
func (builder *MetricsQueryBuilder) AccountId(accountId string) *MetricsQueryBuilder {
	builder.internal.AccountId = &accountId

	return builder
}

// Metric data aggregations over specified periods of time. For detailed definitions of the statistics supported by CloudWatch, see https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Statistics-definitions.html.
func (builder *MetricsQueryBuilder) Statistic(statistic string) *MetricsQueryBuilder {
	builder.internal.Statistic = &statistic

	return builder
}

// When the metric query type is set to `Insights` and the `metricEditorMode` is set to `Builder`, this field is used to build up an object representation of a SQL query.
func (builder *MetricsQueryBuilder) Sql(sql cog.Builder[SQLExpression]) *MetricsQueryBuilder {
	sqlResource, err := sql.Build()
	if err != nil {
		builder.errors = append(builder.errors, err.(cog.BuildErrors)...)
		return builder
	}
	builder.internal.Sql = &sqlResource

	return builder
}

// For mixed data sources the selected datasource is on the query level.
// For non mixed scenarios this is undefined.
// TODO find a better way to do this ^ that's friendly to schema
// TODO this shouldn't be unknown but DataSourceRef | null
func (builder *MetricsQueryBuilder) Datasource(datasource common.DataSourceRef) *MetricsQueryBuilder {
	builder.internal.Datasource = &datasource

	return builder
}

// @deprecated use statistic
func (builder *MetricsQueryBuilder) Statistics(statistics []string) *MetricsQueryBuilder {
	builder.internal.Statistics = statistics

	return builder
}
