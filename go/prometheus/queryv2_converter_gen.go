...
func QueryV2Converter(input dashboardv2.DataQueryKind) string {
	...
	if input.Spec != nil && input.Spec.(*Dataquery).Interval != nil && *input.Spec.(*Dataquery).Interval != "" {

		buffer.WriteString(`Interval(`)
		arg0 := fmt.Sprintf("%#v", *input.Spec.(*Dataquery).Interval)
		buffer.WriteString(arg0)
