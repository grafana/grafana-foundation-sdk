...
func PanelConverter(input dashboard.Panel) string {
	...
	if input.Options != nil {
		...
		arg0 := common.MapLayerOptionsConverter(input.Options.(*Options).Basemap)
		buffer.WriteString(arg0)
	...
	if input.Options != nil && input.Options.(*Options).Layers != nil && len(input.Options.(*Options).Layers) >= 1 {

		buffer.WriteString(`Layers(`)
		tmparg0 := []string{}
		for _, arg1 := range input.Options.(*Options).Layers {
