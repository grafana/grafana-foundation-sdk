...
func ActionConverter(input Action) string {
	calls := []string{
		`dashboardv2beta1.NewActionBuilder()`,
	}
	...
	if input.Variables != nil && len(input.Variables) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2beta1.ActionVariable]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
	...
	if input.Style != nil {
		...
		arg0 := Dashboardv2beta1ActionStyleConverter(*input.Style)
		buffer.WriteString(arg0)
