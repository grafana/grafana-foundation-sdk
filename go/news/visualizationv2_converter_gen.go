...
func VisualizationV2Converter(input dashboardv2.VizConfigKind) string {
	calls := []string{
		`news.NewVisualizationV2Builder()`,
	}
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).FeedUrl != nil && *input.Spec.Options.(*Options).FeedUrl != "" {

		buffer.WriteString(`FeedUrl(`)
		arg0 := fmt.Sprintf("%#v", *input.Spec.Options.(*Options).FeedUrl)
		buffer.WriteString(arg0)
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).ShowImage != nil && *input.Spec.Options.(*Options).ShowImage != true {

		buffer.WriteString(`ShowImage(`)
		arg0 := fmt.Sprintf("%#v", *input.Spec.Options.(*Options).ShowImage)
		buffer.WriteString(arg0)
