...
func MetricQueryConverter(input MetricQuery) string {
	calls := []string{
		`azuremonitor.NewMetricQueryBuilder()`,
	}
	...
	if input.Resources != nil && len(input.Resources) >= 1 {
		...
		for _, arg1 := range input.Resources {
			tmpresourcesarg1 := MonitorResourceConverter(arg1)
			tmparg0 = append(tmparg0, tmpresourcesarg1)
		}
		arg0 := "[]cog.Builder[azuremonitor.MonitorResource]{" + strings.Join(tmparg0, ",\n") + "}"
	...
	if input.DimensionFilters != nil && len(input.DimensionFilters) >= 1 {
		...
		for _, arg1 := range input.DimensionFilters {
			tmpdimensionFiltersarg1 := MetricDimensionConverter(arg1)
			tmparg0 = append(tmparg0, tmpdimensionFiltersarg1)
		}
		arg0 := "[]cog.Builder[azuremonitor.MetricDimension]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
