...
func AdhocVariableConverter(input AdhocVariableKind) string {
	calls := []string{
		`dashboardv2.NewAdhocVariableBuilder(` + fmt.Sprintf("%#v", input.Spec.Name) + `)`,
	}
	...
	if input.Datasource != nil {
		...
		arg0 := Dashboardv2AdhocVariableKindDatasourceConverter(*input.Datasource)
		buffer.WriteString(arg0)
	...
	if input.Spec.BaseFilters != nil && len(input.Spec.BaseFilters) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.AdHocFilterWithLabels]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
	...
	if input.Spec.DefaultKeys != nil && len(input.Spec.DefaultKeys) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.MetricFindValue]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
