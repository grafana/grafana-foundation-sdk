...
func Manifest(name string, spec cog.Builder[Dashboard]) *resource.ManifestBuilder {
	...
	if err != nil {
		builder.RecordError("Manifest(spec)", err)
		return builder
	}
	builder.Spec(specResource)
...
func (builder *DashboardBuilder) Build() (Dashboard, error) {
	...
	if len(builder.errors) > 0 {
		return Dashboard{}, cog.MakeBuildErrors("dashboardv2.dashboard", builder.errors)
	}
...
func (builder *DashboardBuilder) Preferences(preferences cog.Builder[Preferences]) *DashboardBuilder {
	preferencesResource, err := preferences.Build()
	if err != nil {
		builder.errors = append(builder.errors, err.(cog.BuildErrors)...)
		return builder
	}
	builder.internal.Preferences = &preferencesResource
