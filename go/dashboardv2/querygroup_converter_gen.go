...
func QueryGroupConverter(input QueryGroupKind) string {
	calls := []string{
		`dashboardv2.NewQueryGroupBuilder()`,
	}
	...
	if input.Spec.Queries != nil && len(input.Spec.Queries) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.PanelQueryKind]{" + strings.Join(tmparg0, ",\n") + "}"
	...
	if input.Spec.Transformations != nil && len(input.Spec.Transformations) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.TransformationKind]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
