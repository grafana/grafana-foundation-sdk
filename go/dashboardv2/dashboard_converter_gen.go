...
func DashboardConverter(input Dashboard) string {
	calls := []string{
		`dashboardv2.NewDashboardBuilder(` + fmt.Sprintf("%#v", input.Title) + `)`,
	}
	...
	if input.Annotations != nil && len(input.Annotations) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.AnnotationQueryKind]{" + strings.Join(tmparg0, ",\n") + "}"
	...
	if input.Elements != nil {
		...
		arg0 := "map[string]dashboardv2.Element{"
	...
	if input.Links != nil && len(input.Links) >= 1 {
		...
		arg0 := "[]cog.Builder[dashboardv2.DashboardLink]{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
	...
	if input.Preferences != nil {
		...
		buffer.WriteString(`Preferences(`)
		arg0 := PreferencesConverter(*input.Preferences)
		buffer.WriteString(arg0)
