...
func GroupByVariableConverter(input GroupByVariableKind) string {
	calls := []string{
		`dashboardv2.NewGroupByVariableBuilder(` + fmt.Sprintf("%#v", input.Spec.Name) + `)`,
	}
	...
	if input.Datasource != nil {
		...
		arg0 := Dashboardv2GroupByVariableKindDatasourceConverter(*input.Datasource)
		buffer.WriteString(arg0)
	...
	if input.Spec.Options != nil && len(input.Spec.Options) >= 1 {
		...
		arg0 := "[]dashboardv2.VariableOption{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
