...
func ConditionalRenderingGroupConverter(input ConditionalRenderingGroupKind) string {
	calls := []string{
		`dashboardv2.NewConditionalRenderingGroupBuilder()`,
	}
	...
	if input.Spec.Items != nil && len(input.Spec.Items) >= 1 {
		...
		arg0 := "[]dashboardv2.ConditionalRenderingVariableKindOrConditionalRenderingDataKindOrConditionalRenderingTimeRangeSizeKind{" + strings.Join(tmparg0, ",\n") + "}"
		buffer.WriteString(arg0)
