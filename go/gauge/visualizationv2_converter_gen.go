...
func VisualizationV2Converter(input dashboardv2.VizConfigKind) string {
	calls := []string{
		`gauge.NewVisualizationV2Builder()`,
	}
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).ShowThresholdLabels != false {

		buffer.WriteString(`ShowThresholdLabels(`)
		arg0 := fmt.Sprintf("%#v", input.Spec.Options.(*Options).ShowThresholdLabels)
		buffer.WriteString(arg0)
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).ShowThresholdMarkers != true {

		buffer.WriteString(`ShowThresholdMarkers(`)
		arg0 := fmt.Sprintf("%#v", input.Spec.Options.(*Options).ShowThresholdMarkers)
		buffer.WriteString(arg0)
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).MinVizWidth != 75 {
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).MinVizHeight != 75 {
