...
func VisualizationV2Converter(input dashboardv2.VizConfigKind) string {
	calls := []string{
		`debug.NewVisualizationV2Builder()`,
	}
	...
	if input.Spec.Options != nil && input.Spec.Options.(*Options).Counters != nil {

		buffer.WriteString(`Counters(`)
		arg0 := UpdateConfigConverter(*input.Spec.Options.(*Options).Counters)
		buffer.WriteString(arg0)
