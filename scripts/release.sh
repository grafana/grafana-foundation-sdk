...
RELEASE_TAG=${1:-"v0.0.0"} # version being released.
...
# LOG_LEVEL=7 ./scripts/release.sh v0.0.0
...
pr_branch="preview-${build_timestamp}"
...
notice "Release tag: ${RELEASE_TAG}"
...
info "Running cog"
run_codegen "output_dir=${codegen_output_path}/%l,kind_registry_path=${KIND_REGISTRY_PATH},release_tag=${RELEASE_TAG}"
...
run_when_safe gh_run "${foundation_sdk_path}" pr create \
  --base "${release_branch}" \
  --head "${pr_branch}" \
  --title "Automated release for ${RELEASE_TAG}" \
