...
```python
def build() -> dashboardv2.QueryVariableKind
```
...
```python
def refresh(refresh: dashboardv2.VariableRefresh) -> typing.Self
```
...
```python
def regex_apply_to(regex_apply_to: dashboardv2.VariableRegexApplyTo) -> typing.Self
```
...
```python
def sort(sort: dashboardv2.VariableSort) -> typing.Self
```

### <span class="badge object-method"></span> static_options

```python
def static_options(static_options: list[dashboardv2.VariableOption]) -> typing.Self
```
