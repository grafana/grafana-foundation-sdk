---
title: <span class="badge builder"></span> CustomFormatterVariable
---
# <span class="badge builder"></span> CustomFormatterVariable

## Constructor

```python
CustomFormatterVariable()
```
## Methods

### <span class="badge object-method"></span> build

Builds the object.

```python
def build() -> dashboardv2.CustomFormatterVariable
```

### <span class="badge object-method"></span> include_all

```python
def include_all(include_all: bool) -> typing.Self
```

### <span class="badge object-method"></span> multi

```python
def multi(multi: bool) -> typing.Self
```

### <span class="badge object-method"></span> name

```python
def name(name: str) -> typing.Self
```

### <span class="badge object-method"></span> type_val

```python
def type_val(type_val: dashboardv2.VariableType) -> typing.Self
```

## See also

 * <span class="badge object-type-class"></span> [CustomFormatterVariable](./object-CustomFormatterVariable.md)
